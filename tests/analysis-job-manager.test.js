...
async function waitForTerminalState(manager, id) {
...
test('server-owned job processes every symbol', async () => {
    const calls = [];
    ...
    const completed = await waitForTerminalState(manager, created.id);
...
test('cancelling a superseded app-tab job stops before the next symbol', async () => {
    const calls = [];
    let releaseFirst;
    ...
    const manager = new AnalysisJobManager({
        ...
        analyze: async symbol => {
            calls.push(symbol);
            return symbol === 'IDC' ? firstResult : { symbol };
        }
    });
    ...
    releaseFirst({ symbol: 'IDC' });
    const cancelled = await waitForTerminalState(manager, created.id);

    assert.equal(cancelled.status, 'cancelled');
    assert.deepEqual(calls, ['IDC']);
});
